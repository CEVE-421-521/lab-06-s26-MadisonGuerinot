...
For the low SLR scenario, the optimal elevation is 0, while the optimal decision for the high scenario is close to 7. this makes sense becasue under the low scenario, the expected damages are much lower, so construction costs are not justified. 

2. *Why are the cost curves U-shaped? What happens at very low vs. very high elevation?*

At very low elevation, the expected damages are unmitigated while the construction cost remains low. At the high end, this is reversed, resulting in a U-shaped curve.
...
EVPI = minimum(costs_prior) - (p_low * minimum(costs_low) + p_high * minimum(costs_high))
```
...
DataFrame(
    EVPI = EVPI,
    EVPI_as_fraction_of_house_value = EVPI / house.value_usd,
)
...
1. *Interpret the EVPI in dollars. How much would this homeowner pay for a perfect forecast of the future emissions pathway?* A homeowner would be willing to pay 2,2047 dollars for a perfect forecast of the future emissions pathway. 

2. *What fraction of the house value ($250,000) does the EVPI represent?*
The EVPI represents 8.8% of the house value.
...
This EVPI is large enough to justify funding climate policy research, especially considering that the same information would benefit millions of homeowners in coastal areas around the world who face similar SLR risks. The aggregated value of this information across all affected homeowners could be substantial, making it a worthwhile investment for society as a whole.
...
```{julia}
p_offtrack = p_offtrack_given_85 * p_high + (1 - p_ontrack_given_26) * p_low
p_ontrack = (1 - p_offtrack_given_85) * p_high + p_ontrack_given_26 * p_low

p_85_given_offtrack = (p_offtrack_given_85 * p_high) / p_offtrack
p_26_given_ontrack = (p_ontrack_given_26 * p_low) / p_ontrack
```
...
```{julia}
expected_with_posterior_info = p_offtrack * minimum(p_85_given_offtrack * costs_high .+ (1 - p_85_given_offtrack) * costs_low) + p_ontrack * minimum(p_26_given_ontrack * costs_low .+ (1 - p_26_given_ontrack) * costs_high)

EVII = minimum(costs_prior) - expected_with_posterior_info
```
...
DataFrame(
    Scenario = ["No Info", "Policy Report", "Perfect Info"],
    Cost = [minimum(costs_prior), expected_with_posterior_info, p_low * minimum(costs_low) + p_high * minimum(costs_high)],
    Value_of_Info = [0.0, EVII, EVPI],
)
```
...
let
    ...
    ax = Axis(
        ...
        xlabel="Elevation Height (ft)",
        ylabel="Total Cost (USD)",
        title="Cost Curves with Policy Report Signals",
    )

    # Re-weight cost curves with posteriors
    costs_offtrack = p_85_given_offtrack * costs_high .+ (1 - p_85_given_offtrack) * costs_low
    costs_ontrack = p_26_given_ontrack * costs_low .+ (1 - p_26_given_ontrack) * costs_high

    lines!(ax, height_grid, costs_offtrack; color=:green, linewidth=2, label="Report: 'Off Track'")
    lines!(ax, height_grid, costs_ontrack; color=:orange, linewidth=2, label="Report: 'On Track'")

    # Mark optima
    idx_offtrack = argmin(costs_offtrack)
    idx_ontrack = argmin(costs_ontrack)

    scatter!(ax, [height_grid[idx_offtrack]], [costs_offtrack[idx_offtrack]]; color=:green, markersize=15, marker=:star5)
    scatter!(ax, [height_grid[idx_ontrack]], [costs_ontrack[idx_ontrack]]; color=:orange, markersize=15, marker=:star5)
...
The policy report captures approximately 50% of the EVPI, indicating that while it provides valuable information, it is not as informative as perfect information.

Why is EVII < EVPI?
Because the policy report is imperfect and does not perfectly predict the future emissions pathway, it cannot capture all the value of perfect information. The inaccuracies in the report reduce its ability to change the optimal decision as effectively as perfect information would, resulting in a lower EVII compared to EVPI.

What would make the report more valuable?
The report would be more valuable if it had higher accuracy in predicting the future emissions pathway, or if it provided more precise information about the likelihood of different scenarios. Additionally, if the report could be updated more frequently as new data becomes available, it could help homeowners make more informed decisions over time, increasing its value.
::!
...
*Based on your EVPI result, what is the maximum value of investment (1)? The maximum value of investment (1) is equal to the EVPI, which represents the maximum amount the homeowner would be willing to pay for perfect information about the future emissions pathway. This value can be directly compared to the cost of funding the climate policy analysis to determine if it is a worthwhile investment.*
Why might investment (2) have a different (possibly larger or smaller) ceiling?*
Investment (2) might have a different ceiling because it addresses a different source of uncertainty (storm surge) that may have a different impact on the homeowner's decision compared to the uncertainty about future emissions pathways. The value of reducing storm surge uncertainty would depend on how much it changes the expected damages and optimal elevation decision, which could be more or less significant than the value of knowing the future emissions pathway. Therefore, the maximum value of investment (2) could be larger or smaller than that of investment (1) based on its influence on the decision-making process and potential cost savings.
:::
